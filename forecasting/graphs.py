...
from .naive_method import naive_forecaster, forecast_dates, bs_pi_output, Naive_forecast
from .drift_method import drift_forecaster, Drift_forecast
from .mean_method import mean_forecaster, Mean_forecast
...
def fitted_forecast_graph(df:pd.DataFrame,target_col:str,model) -> go.Figure:
    ...
    fig.update_layout(height=600,
                    title_text=f'Observed data and {type(model).__name__} fitted forecast',
...
def decomp(df:pd.DataFrame, target_col:str, period, **MSTLkwargs) -> tuple[pd.Series]:
    ...
    Parameters:
        df: pandas.DataFrame - Historical time series data with date-time index.
        target_col: str - Column with historical data.
        period: int ot list - Seasonal period.
        **MSTLkwargs - Keyword arguments for the statsmodels MSTL class.
...
def decomp_plot(df:pd.DataFrame, target_col:str, period, **MSTLkwargs) -> go.Figure:
    ...
    Parameters:
        df: pandas.DataFrame - Historical time series data with date-time index.
        target_col: str - Column with historical data.
        period: int or list - Seasonal period.
        **MSTLkwargs - Keyword arguments for the statsmodels MSTL class.
...
def future_forecast_data(df:pd.DataFrame, target_col:str, output_forecast:pd.DataFrame, fill=True) -> go.Figure:
    ...
    Parameters:
         df: pandas.DataFrame - Historical time series data with date-time index.
         target_col: str - Column with historical data.
         output_forecast: pandas.DataFrame - Data frame with the desired forecast and the lower and upper bounds of the prediction intervals as columns,
                                                   as outputted from the benchmark forecasts.
         fill: bool - Toggle whether to fill the space between each prediction interval.
...
sim_dict = {'naive': naive_forecaster, 'drift' : drift_forecaster, 'mean' : mean_forecaster}


def bootstrap_sim_graph(df:pd.DataFrame, target_col:str, horizon:int, model:str, bootstrap_samples:int=100, pred_width:list = [95,80], **kwargs) -> go.Figure:
    """
    ...
    Parameters:
        ...
        target_col: str - Column with historical data.
        horizon: int - Number of timesteps forecasted into the future.
        model: str - One of {'naive','drift','mean'}, the model to simulate the forecast.
        ...
        **kwargs - Arguments used for the chosen model, i.e. period for 'naive' and window for 'mean
    ...
    method = sim_dict[model]

    bs_simulations = method(df = df,
                            target_col = target_col,
                            horizon = horizon,
    ...
    for sample in range(bootstrap_samples):
        ...
        bs_fig.add_trace(go.Scatter(x=bs_simulations.index, y=bs_simulations[f'sim_{sample}'],
                                    showlegend=False,
    ...
    output_forecast = bs_pi_output(forecast_data = bs_simulations,
...
def cross_val_graph(df:pd.DataFrame,target_col:str,models:dict = None, period:int=1, n_splits:int=5,test_size:int=None) -> go.Figure:
    ...
    Parameters:
        df: pandas.DataFrame - Historical time series data with date-time index.
        target_col:str - Column with historical data.
        models: dict - The models to compare the cross validation of. Defaults to None,
    ...
    model_dict = {'naive': Naive_forecast(period = period, pred_width=None),
